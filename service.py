...
def Search( item ):
    ...
    log( __name__ ,"LOG:1: [%s]" % (url,))
    #path name
    dirname = os.path.basename( os.path.dirname(item['file_original_path']) )
    sname = ''
    se = ''
    ...
    if sp!=None :
        ...
        se = sp.group()
        #监测上级Season
        path2 = os.path.dirname( os.path.dirname(item['file_original_path']) )
        dirname = os.path.basename(path2)
        ...
        if sp==None :
            ...
            #包含season
            sname = os.path.basename(os.path.dirname( path2 ))
    else:
        #不包含 S1E1 剧集信息
        sp = re.search('season[ -_+~#^()=]*[0-9]{1,}', dirname.lower())
        if sp==None :
            ...
            #包含season
            fname = os.path.basename( item['file_original_path'] )
            sp = re.search('s[0-9]{1,}e[0-9]{1,}', fname.lower())
            if sp!=None :
                se = sp.group()
            sname = os.path.basename( os.path.dirname( os.path.dirname(item['file_original_path']) ))
    if se:
        sname = '%s %s'%(sname, se)
        log( __name__ ,"LOG:3: [%s][%s]" % (sname,se,))
    #url
    url = SUBHD_API % (urllib.quote(sname.encode('utf-8')))
    log( __name__ ,"LOG:2: [%s][%s]" % (url,se,))
...
def Download(url,lang):
    ...
    for subfile in files:
        if (os.path.splitext( subfile )[1] in exts):
            fname = subfile.decode('utf-8')
            list.append(fname)
            fext = os.path.splitext( subfile )[1].strip('.').upper()
            show = '[%s] %s'%(fext, fname)
    ...
    if len(list) == 1:
        ...
        sel = xbmcgui.Dialog().select('请选择压缩包中的字幕', showlist)
        if sel == -1:
            sel = 0
